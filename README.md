...
So, I did the lazy thing and just filtered my search on the Qasa website, then copied the GraphQL query it generated, and added it as a HTTP POST here in the code. This is sent every 60 seconds, giving me a JSON list of the properties for rent. 

Using some vibe-coding, I then simply parsed and extracted the JSON fields I wanted, made a listing object for each listing, and then formated this to be sent as a bot message in a server channel in Discord.
...
2. Edit 'query' in getListings() to be whatever you want by.
...
I've asked, and Qasa have kindly given their approval as long as the User-Agent in the code is included and kept the same. This is, of course, subject to change upon request in the future. 

As a common courtesy, if you are going to use this, please don't ping the backend every second, that's both pointless, and will flood their site for no reason (60 seconds is acceptable)!
